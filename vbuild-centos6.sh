...
if build "vapoursynth"; then
	download "https://github.com/vapoursynth/vapoursynth/archive/master.zip" "vapoursynth.zip"
	cd $PACKAGES/vapoursynth*
	execute ./autogen.sh
	execute ./configure --prefix=${WORKSPACE} --enable-shared
	execute make
	execute make install
